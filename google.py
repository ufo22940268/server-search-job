...
class Google:
    ...
    """
    @staticmethod
    def search(query, pages = 1, append_url = None):
        results = []
        for i in range(pages):
            url = get_search_url(query, i, append_url = append_url)
            html = get_html(url)
            if html:
                ...
                for li in lis:
                    ...
                    res.link = a["href"]
                    res.link = res.link.replace('/url?q=', '')
                    
                    #Remove useless params in link
                    res.link = re.sub(r'&sa.+', '', res.link)
    ...
    def parse_email(url):
        ...
        content = soup.text
        match = re.search(r'[\w\.-]+@[\w\.-]+(com|org|me)', content)
        if match:
...
def get_search_url(query, page = 0, per_page = 10, append_url = None):
    # note: num per page might not be supported by google anymore (because of google instant)
    url = "http://www.google.com/search?hl=zh-CN&q=%s&start=%i&num=%i" % (normalize_query(query), page * per_page, per_page)
    if append_url:
        url = url + append_url
    return url
...
def my_test():
    # print Google.search(u'杭州 android 招聘', append_url = '&tbs=qdr:w')[0].name
    rs = Google.search(urllib2.quote('杭州 android 招聘'), append_url = '&tbs=qdr:w')
    # print rs[0].email
    print [x.email for x in rs]
    # print Google.parse_email('http://www.cocoachina.com/bbs/read.php?tid=183891')
